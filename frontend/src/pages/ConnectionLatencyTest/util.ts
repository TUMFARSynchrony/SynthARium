...
export function median(arr: number[]) {
  ...
  if (arr.length !== filtered.length) {
    console.warn(
      "Ignoring",
      arr.length - filtered.length,
      "undefined values in input for median()"
    );
  }
...
export function calculateEvaluation(
  ...
  if (log) {
    ...
    console.log(
      `Invalid Latency Data Points: ${invalidLatencyDataPoints} (${invalidLatencyDataPointsPercent}%)`
    );
    ...
    console.log(
      `Missing QR Code Generation Data Points: ${missingQRCodeGenDataPoints} (${missingQRCodeGenDataPointsPercent}%)`
    );
    ...
    if (hasPingData) {
      ...
      console.log(
        `Missing Ping Data Points: ${ping.missingPingDataPoints} (${ping.missingPingDataPointsPercent}%)`
      );
